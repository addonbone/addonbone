...
export default class ExpressionFile extends SourceFile {
    ...
    public getType(): string | undefined {
        // Find export default and parse its expression into member signatures
        ...
        // Build interface-like string
        const parts = Object.entries(members).map(([key, sig]) => {
            if (sig.kind === 'method') {
                // include generic parameters if any
                const tp = sig.typeParameters && sig.typeParameters.length
        ...
        // optionally prefix with a space for easier matching when sufficiently many members
    ...
    private inlineAliasType(name: string): string | undefined {
        ...
        if (local) {
            return this.resolveTypeNode(local.type);
        }
        // imported alias
        const importPath = this.getImports().get(name);
        if (importPath) {
