...
import {LocaleNonPluralKeysOf, LocaleSubstitutionsFor, Language, LanguageCodes, LocaleDir} from "@typing/locale";
...
export const __ = (key: Extract<keyof LocaleNativeStructure, string>): string => {
    if (!NativeLocale.getInstance().keys().has(key)) {
        console.warn(`Locale key "${key as string}" not found in "${NativeLocale.getInstance().lang()}" language.`);
    }
 ...
 *
 * @param {string} key - The input string from which a locale key is to be extracted.
